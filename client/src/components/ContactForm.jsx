        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {[
            ...
            <div key={item.label} className="bg-card border border-primary/30 rounded-xl p-6 glow-cyan">
              ...
              {item.href ? (
                <a href={item.href} className="text-primary hover:text-accent transition-colors">{item.value}</a>
              ) : (
        ...
        <div className="bg-card border border-primary/30 rounded-2xl p-8 glow-cyan">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Loader2 size={20} className="animate-spin" /> Sending...</> : 'Send Message'}
            </button>
          </form>
        </div>
      </div>
